...
namespace KeweenawCheeseSite.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var discriminator = claims.First(x => x.Type =="urn:discord:user:discriminator")
                        .Value;

                var avatar = claims.First(x => x.Type == "urn:discord:avatar:hash")
                        .Value;

                var user = new DiscordUser()
                {
                    Id = Int64.Parse(info.ProviderKey),
